...
class Browser {
	...
	*/
	...
	public function getShortversion( $howlong = 0 )
	{
		if ( $this->getVersion() == 'unknown' ) {
			$browser_shortversion = '0';
		} else {
			$separator = "";
			$numbers = explode( ".", $this->getVersion() );
			$first = $numbers['0'];
			$last = $numbers['1'];
			if ( $howlong > 0 ) {
				$separator = "_";
			}
			$browser_shortversion = $first . $separator . substr($last, 0, $howlong);
		}

		return $browser_shortversion;
	}
	...
	public function custom_browser_info()
	{
		...
		if( $this->getBrowser() == 'Internet Explorer' ) {
			...
			$browser_type = strtolower( $this->getBrowser() );
		}

		if ( $this->getVersion() == 'unknown' ) {
			$browser_version = '0';
		} else {
			$browser_version = str_replace('.', '_', $this->getVersion() );
		...
		if ( $this->getVersion() == 'unknown' ) {
			...
			$browser_shortversion = str_replace('.', '_', $this->getVersion() );
			$browser_shortversion = substr($browser_shortversion, 0, 1);
		}

		$output = strtolower( $this->getPlatform() ) . ' ';
		if ( $this->getBrowser() == 'Internet Explorer' ) {
			$output .= $browser_type . ' ';
		...
		if ( ( ( $this->getBrowser() == 'Internet Explorer' ) && ( $browser_shortversion == '6' ) ) || ( ( $this->getBrowser() == 'Internet Explorer' ) && ( $browser_shortversion == '7' ) ) ) {
			$output .= ' ie6-7';
		}
		if ( ( ( $this->getBrowser() == 'Internet Explorer' ) && ( $browser_shortversion == '6' ) ) || ( ( $this->getBrowser() == 'Internet Explorer' ) && ( $browser_shortversion == '7' ) ) || ( ( $this->getBrowser() == 'Internet Explorer' ) && ( $browser_shortversion == '8' ) ) ) {
			$output .= ' ie6-8';
		}
